...
function filtro($tabela)
{
    ...
    $product_companyResult = runQuery("SELECT DISTINCT product_company FROM Allproducts WHERE product_category = '$tabela' ORDER BY product_company ASC");
...
<div id=\"demo\" style=\"display: none;\">
        ...
        <div class=\"form-price-range-filter\">
            <form id=\"filter-form\" name=\"search\" class=\"filter-form\" method=\"POST\">
                <div class=\"form-group\">
                    <input type=\"\" id=\"min\" name=\"min_price\" value=\"$min\" readonly=\"readonly\">
                    <div id=\"slider-range\"></div>
                    <input type=\"\" id=\"max\" name=\"max_price\" value=\"$max\" readonly=\"readonly\">
                </div>
    ...
    if (!empty($product_companyResult)) {
        foreach ($product_companyResult as $key => $value) {
            echo '<option value="' . $product_companyResult[$key]['product_company'] . '">' . $product_companyResult[$key]['product_company'] . '</option>';
        }
    }
    echo "</select>
</div>
...
function pricerange($tabela, $faqja)
{
    ...
    if (!empty($_POST['product_oferta'])) {
        ...
        if (!empty($result)) {
            foreach ($result as $key => $value) {
                component($result[$key]['product_name'], $result[$key]['product_price'], $result[$key]['product_image'], $result[$key]['id'], $result[$key]['product_ofertangjyra'], $result[$key]['product_oferta'], $result[$key]['product_category'], $result[$key]['product_company'], $result[$key]['product_description'], $result[$key]['product_earlierprice']);
            }
        }
    ...
    if (!empty($_POST['product_company'])) {
        $i = 0;
        $selectedOptionCount = count($_POST['product_company']);
        $selectedOption = "";
        while ($i < $selectedOptionCount) {
            $selectedOption = $selectedOption . "'" . $_POST['product_company'][$i] . "'";
            if ($i < $selectedOptionCount - 1) {
                $selectedOption = $selectedOption . ", ";
            }
            $i++;
        }
        $query = "SELECT * from Allproducts WHERE (product_category = '$tabela' and product_price BETWEEN '$min' AND '$max' and product_company in (" . $selectedOption . "))";
